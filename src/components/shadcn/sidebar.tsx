...
// Enhancements - keyboard shortcut for cart, cart width
...
// ToDo: merge this
function useRightSidebarContent() {
	const { rightSidebarContent, setRightSidebarContent } = useSidebar()
	return { content: rightSidebarContent, setContent: setRightSidebarContent }
}
...
function SidebarProvider({
	...
	const setOpen = useCallback(
		(value: boolean | ((value: boolean) => boolean)) => {
			...
			// biome-ignore lint/suspicious/noDocumentCookie: ShadCN code, not mine
	...
	const contextValue = useMemo<SidebarContextProps>(
		...
		[state, open, setOpen, isMobile, openMobile, toggleSidebar, rightSidebarOpen, toggleRightSidebar, rightSidebarContent],
	)
	...
	return (
		<SidebarContext.Provider value={contextValue}>
			<div
				...
				className={mergeClasses('group/sidebar-wrapper has-data-[variant=inset]:bg-sidebar flex min-h-svh w-full', className)}
				{...props}
			>
				{children}
			</div>
...
function Sidebar({
	...
	const { isMobile, state, openMobile, setOpenMobile, rightSidebarOpen, setRightSidebarOpen } = useSidebar()

	// Use appropriate state based on sidebar side
	const sidebarOpen = side === 'right' ? rightSidebarOpen : isMobile ? openMobile : state === 'expanded'
	...
	if (isMobile) {
		// For mobile, use the appropriate open state and setter based on sidebar side
		const mobileOpen = side === 'right' ? rightSidebarOpen : openMobile
		const setMobileOpen = side === 'right' ? setRightSidebarOpen : setOpenMobile
		...
		return (
			<Sheet open={mobileOpen} onOpenChange={setMobileOpen} {...props}>
				<SheetContent
					...
					className={
						side === 'right'
							? 'bg-sidebar text-sidebar-foreground w-(--sidebar-width-right) p-0 [&>button]:hidden'
	...
	return (
		...
		>
			...
			<div
				...
				className={mergeClasses(
					side === 'right'
						? 'fixed inset-y-0 z-sidebar hidden h-svh w-(--sidebar-width-right) transition-[left,right,width] duration-200 ease-linear md:flex'
...
function SidebarTrigger({ className, onClick, ...props }: ComponentProps<typeof Button>) {
	...
	return (
		<TooltipWrapper>
			...
			<TooltipContent>{open ? 'Hide menu' : 'Show menu'}</TooltipContent>
		</TooltipWrapper>
	)
}

function CartSideBarTrigger({ className, onClick, ...props }: ComponentProps<typeof Button>) {
	...
	return (
		<TooltipWrapper>
			...
			<TooltipContent>{rightSidebarOpen ? 'Hide sidebar' : 'Show sidebar'}</TooltipContent>
		</TooltipWrapper>
	)
}
...
function SidebarHeader({ className, ...props }: ComponentProps<'div'>) {
	return <div data-slot="sidebar-header" data-sidebar="header" className={mergeClasses('flex flex-col gap-2 p-2', className)} {...props} />
}

function SidebarFooter({ className, ...props }: ComponentProps<'div'>) {
	return <div data-slot="sidebar-footer" data-sidebar="footer" className={mergeClasses('flex flex-col gap-2 p-2', className)} {...props} />
}
...
function SidebarContent({ className, ...props }: ComponentProps<'div'>) {
	return (
		<div
			...
			className={mergeClasses('flex min-h-0 flex-1 flex-col gap-2 overflow-auto group-data-[collapsible=icon]:overflow-hidden', className)}
			{...props}
		/>
	)
}
...
function SidebarGroupContent({ className, ...props }: ComponentProps<'div'>) {
	return <div data-slot="sidebar-group-content" data-sidebar="group-content" className={mergeClasses('w-full', className)} {...props} />
}

function SidebarMenu({ className, ...props }: ComponentProps<'ul'>) {
	return (
		<ul data-slot="sidebar-menu" data-sidebar="menu" className={mergeClasses('flex w-full min-w-0 flex-col gap-1', className)} {...props} />
	)
}

function SidebarMenuItem({ className, ...props }: ComponentProps<'li'>) {
	return (
		<li data-slot="sidebar-menu-item" data-sidebar="menu-item" className={mergeClasses('group/menu-item relative', className)} {...props} />
	)
}
